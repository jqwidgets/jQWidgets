...
const App = () => {
    const productNumber = useRef<HTMLParagraphElement>(null);
    const itemsInCart = useRef<number>(0);
    ...
    const rendered = useCallback(() => {
        const clickHandler = () => {
            itemsInCart.current++;
            if (productNumber.current) {
                productNumber.current.innerHTML = `${itemsInCart.current} products`;
            }
        };

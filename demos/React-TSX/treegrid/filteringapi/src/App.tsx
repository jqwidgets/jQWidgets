...
const sourceDropdownlist: IDropDownListProps["source"] = [
  { label: 'First Name', value: 'FirstName' },
  { label: 'Last Name', value: 'LastName' },
  { label: 'City', value: 'City' }
];

const columns: ITreeGridProps["columns"] = [
...
const App = () => {
  ...
  const [treeGridSource] = useState<any>(() => new jqx.dataAdapter(source));
  const [treeGridColumns] = useState(columns);
  const [dropdownSource] = useState(sourceDropdownlist);
  ...
  const updatemyListBox = useCallback((dataField: string) => {
    ...
    const uniqueRecords: any[] = myListBoxAdapter.records;
  ...
  const myDropDownListOnSelect = useCallback((event: any) => {

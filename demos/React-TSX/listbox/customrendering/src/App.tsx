...
function App() {
    const myListBox = React.useRef<JqxListBox>(null);
    const contentPanel = React.useRef<HTMLDivElement>(null);
    ...
    const renderer = React.useCallback((index: number, label: string, value: any) => {
        const datarecord = getData()[index!];
    ...
    const updatePanel = React.useCallback((index: number) => {
        const datarecord = getData()[index];
        const listBoxItemValue = myListBox.current!.getItem(index).value;
        const container = (
        ...
        ReactDOM.render(container, contentPanel.current!);
    }, []);

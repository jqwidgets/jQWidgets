(function Data() {
	const csvData = 
	`Order ID,Month,Customer,Region,State,City,Salesperson,Payment Method,Category,Product Name,Unit Price,Quantity,Revenue,Shipping Cost
			...
			columns: [
				{
					...
					template: function(formatObject) {
						if (!formatObject.template) {					
							const salesValue = '<span style="padding: 4px 10px; border-radius: 10px; color: #fff; background: ' + getChipColor() + '";>' + formatObject.row.data['Salesperson'] + '</span>';
							const categoryValue = '<span style="margin-left: 10px; padding: 4px 10px; border-radius: 10px; color: #fff; background: ' + getChipColor() + '";>' + formatObject.row.data['Category'] + '</span>';
							const productValue = '<span style="margin-left: 10px; padding: 4px 10px; border-radius: 10px; color: #fff; background: ' + getChipColor() + '";>' + formatObject.value + '</span>';
